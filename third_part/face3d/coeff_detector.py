...
# Pritaikyti SOLID principai: SRP (Single Responsibility Principle) ir OCP (Open/Closed Principle)
...
class ModelHandler(IModelHandler):
    ...
    def pred_coeffs_dict(self):
        return {
            key: self.model.pred_coeffs_dict[key].cpu().numpy() for key in self.model.pred_coeffs_dict
        }

class ImageTransformer:
    def __init__(self, lm3d_std):
        self.lm3d_std = lm3d_std

    def transform_image(self, images, lm):
        W, H = images.size
        if np.mean(lm) == -1:
...
class CoeffDetector(nn.Module):
    def __init__(self, model_handler: IModelHandler, image_transformer: ImageTransformer):
        super().__init__()
        self.model_handler = model_handler
        self.image_transformer = image_transformer

    def forward(self, img, lm):
        img, trans_params = self.image_transformer.transform_image(img, lm)
        data_input = {
            'imgs': img[None],
        }
        self.model_handler.set_input(data_input)
        self.model_handler.test()

        pred_coeff = {key: self.model_handler.pred_coeffs_dict[key] for key in self.model_handler.pred_coeffs_dict}
...
if __name__ == "__main__":
    ...
    model_handler = ModelHandler(opt)
    image_transformer = ImageTransformer(load_lm3d(opt.bfm_folder))
    coeff_detector = CoeffDetector(model_handler, image_transformer)
