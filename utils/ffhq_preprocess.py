...
class Croper:
    ...
    def get_landmark(self, img_np):
        ...
        lm = np.array([[part.x, part.y] for part in shape.parts()])
        return lm
...
def crop_single_frame(self, img_np, crop, quad):
    clx, cly, crx, cry = crop
    lx, ly, rx, ry = quad
    lx, ly, rx, ry = int(lx), int(ly), int(rx), int(ry)
    cropped_img = img_np[cly:cry, clx:crx]
    cropped_img = cropped_img[ly:ry, lx:rx]
    return cropped_img

def crop_all_frames(self, img_np_list, crop, quad):
    cropped_frames = [self.crop_single_frame(frame, crop, quad) for frame in img_np_list]
    return cropped_frames

def crop(self, img_np_list, xsize=512):
    idx = 0
    ...
    cropped_frames = self.crop_all_frames(img_np_list, crop, quad)
    return cropped_frames, crop, quad
